...
import 'package:qapp/app/features/cutinspection/widgets/info_card.dart';
...
class CutInspection extends StatefulWidget {
  static const String path = "CutInspection";
  const CutInspection({Key? key}) : super(key: key);
  @override
  CutInspectionState createState() => CutInspectionState();
}

class CutInspectionState extends State<CutInspection> {
  CutInspectionViewModel views = CutInspectionViewModel();
  ...
  @override
  void initState() {
    // final views = CutInspectionViewModel();
    // final views = Provider.of<CutInspectionViewModel>(context, listen: false);
    final views = Provider.of<CutInspectionViewModel>(context, listen: false);
    getRoleCount();
    getRoleAccess();

    Future.delayed(Duration.zero, () {
      views.onGetInit(context);
    });
    super.initState();
  }
  ...
  Widget build(BuildContext context) {
    final views = Provider.of<CutInspectionViewModel>(
      context,
    );
    // return Scaffold(
    //     body: SafeArea(
  ...
  Widget _menuTwoModelPopup(context, String titleString1, String titleString2,
    ...
    return BootstrapContainer(
        ...
        children: <Widget>[
          BootstrapRow(
            height: 60,
            children: <BootstrapCol>[
              BootstrapCol(sizes: 'col-3', child: const AbsorbPointer()),
              BootstrapCol(sizes: 'col-6', child: const AbsorbPointer()),
              BootstrapCol(
                sizes: 'col-3',
              ...
              //   sizes: 'col-3',
              //   child: AbsorbPointer(
              //     absorbing: (views.saveWashData.orderNo != null &&
              //             views.currentOptionSelect.isNotEmpty)
              //         ? false
              //         : true,
              //     child: Opacity(
              //       opacity: (views.saveWashData.orderNo != null &&
              //               views.currentOptionSelect.isNotEmpty)
              //           ? 1.0
              //           : 0.4,
              //       child: views.saveWashData.wType != 'A'
              //           ? ActionCard(
              //               views: views,
              //             )
              //           : ApprovalActionCard(views: views),
              //     ),
              //   ),
              //   //       )),
              //   // ))
