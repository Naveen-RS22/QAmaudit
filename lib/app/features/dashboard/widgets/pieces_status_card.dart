  ...
  Widget build(BuildContext context) {
    Defectpscdetail? viewData =
        views.timeSpent.data?.defectpscdetail?.length == 1
            ...
            : null;
    int checkedPcs = viewData != null ? viewData.checkps ?? 0 : 0;
    int passPcs = viewData != null ? viewData.passpcs ?? 0 : 0;

    //List defectPeices = views.timeSpent.data?.defectpscdetail ?? [];
    // int defectPcs = views.timeSpent.data?.defectpscdetail?[0].defectpsc ?? 1;
    int defectPcs = viewData != null ? viewData.defectpsc ?? 0 : 0;
              ...
              children: [
                ...
                Container(
                  ...
                  child: Center(
                    child: Text(
                      isPassCard
                          ? passPercent.round().toString() + "%"
                          : defectPercent.round().toString() + "%",
                      style: TextStyle(
                          color: isPassCard
