...
class FitDashboardViewModel extends BaseViewModel {
  final FitDashboardUseCase _useCase =
      AppManager.instance.fitDashboardUseCase();
  ...
  String currentAuditType = "FAF";

  void currentAuditTypeOnchange(BuildContext context, String type) {
    currentAuditType = type;
    currentPageNotifierOnChange(0);
    onGetAuditSummary(context);
    notifyListeners();
  }

  void currentPageNotifierOnChange(int index) {
  ...
  void onGetAuditSummary(BuildContext context) async {
    ...
    getAudit(context, unitCode, currentDate, currentAuditType);
    getFinalAudit(context, unitCode, currentAuditType, currentDate, userN);
    getTimeline(context, unitCode, currentAuditType, currentDate, userN);
    getDefectPart(context, unitCode, currentAuditType, currentDate, userN);
  ...
  getAudit(context, String unitCode, String currentDate, currentAuditType) {
    auditSummary = AuditSummaryModel();
    notifyListeners();
    _useCase.getAuditSummary(unitCode, currentDate, currentAuditType, (data) {
      auditSummary = data;
  ...
  getTimeline(context, unitcode, audittype, auditDate, auditername) {
    timeSpent = TimeSpentModel();
    notifyListeners();
    _useCase.getTimeSpendModel(unitcode, audittype, auditDate, auditername,
        (data) {
      timeSpent = data;
      notifyListeners();
  ...
  getFinalAudit(context, unitcode, audittype, auditDate, auditername) {
    ...
    filterKeys = [];
    notifyListeners();
    _useCase.getFinalAuditReport(unitcode, audittype, auditDate, auditername,
        (data) {
      //
      finalAudit = data;
  ...
  getSchduleList(context, String unitCode, String currentDate) {
    ...
    _useCase.getScheduleList(unitCode, currentDate, (data) {
      ...
      if (scheduleList.data?.isNotEmpty ?? false) {
        List<StyleAuditData> datas = scheduleList.data!
            .where((o) => o.auditType == currentAuditType)
            .toList();
        scheduleList.data = [];
  ...
  getDefectPart(context, unitcode, audittype, auditDate, auditername) {
    defectPart = DefectPartModel();
    notifyListeners();
    _useCase.getDefectpart(unitcode, audittype, auditDate, auditername, (data) {
      defectPart = data;
      notifyListeners();
      dismissProgress();
    }, (errorMessage) {
