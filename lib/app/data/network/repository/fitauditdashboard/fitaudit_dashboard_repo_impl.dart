...
import 'fitaudit_dashboard_repo.dart';

class FitDashboardRepositoryImpl implements FitDashboardRepository {
  ...
  Future<ApiResult<AuditSummaryModel>> getAuditSummary(
      auditorId, date, auditorName, currentAuditType) async {
    ...
    apiData = {
      ...
      "appEndpoints":
          "${ApiConstants.auditSummary}$auditorId/$date/$auditorName/$currentAuditType",
    };
  ...
  Future<ApiResult<FinalAuditModel>> getFinalAuditStatus(
      unitcode, audittype, auditDate, auditername) async {
    SharedPreferences prefs = await SharedPreferenceHelper.prefs;
    ...
    apiData = {
      ...
      "entityCode": prefs.getString(Constants.entityCode),
      "appEndpoints":
          "${ApiConstants.FinalFitAuditStatus}$unitcode/$audittype/$auditDate/$auditername",
    };
    try {
      final res = await _apiService?.get('', data: apiData);
  ...
  Future<ApiResult<TimeSpentModel>> getTimeSpentDetail(
    ...
    apiData = {
      ...
      "entityCode": prefs.getString(Constants.entityCode),
      "appEndpoints":
          "${ApiConstants.GetFitAuditTimeSpentDetailnew}$unitcode/$audittype/$auditDate/$auditername",
    };
    try {
      final res = await _apiService?.get('', data: apiData);
  ...
  Future<ApiResult<DefectPartModel>> getdefectByParts(
    ...
    apiData = {
      ...
      "entityCode": prefs.getString(Constants.entityCode),
      "appEndpoints":
          "${ApiConstants.GetFitAuditdefectByParts}$unitcode/$audittype/$auditDate/$auditername",
    };
    try {
      final res = await _apiService?.get('', data: apiData);
